...
func begin_hard_game():
	timer = get_node("Timer")
	timer.set_wait_time(get_tree().get_root().get_node("Game").emit_time)
	timer.start()
	limit = 3
	
func begin_harder_game():
	timer = get_node("Timer")
	timer.set_wait_time(get_tree().get_root().get_node("Game").emit_time)
	timer.start()
	limit = 6

func begin_hardest_game():
	timer = get_node("Timer")
	timer.set_wait_time(get_tree().get_root().get_node("Game").emit_time)
	timer.start()
	limit = 9
...
func _on_Timer_timeout():
	...
	randomize()
	var projectile = choices[int(rand_range(0,limit))].instance()
	...
	if (get_tree().get_root().get_node("Game").game_over == false):
